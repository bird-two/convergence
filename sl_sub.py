...
parser.add_argument('--local-epochs', default=1, type=float, metavar='N', help='number of local epochs to run')
...
# hyperparameters
rounds = args.epochs
if args.local_epochs >=1.0:
    local_epochs = args.local_epochs
else:
    local_epochs = 1
    proportion = args.local_epochs
...
def main():
    ...
    sub_idxs_train = sub_local_dataset(localdataset_idxs_train)
    ...
    for i in range(len(sub_idxs_train)):
        train_loaders.append(DataLoader(SubDataset(train_dataset, sub_idxs_train[i]), batch_size=args.batch_size, shuffle=True))
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False)
        
    # Dataset size for the client-size -- client wise averaging   
    datasetsize_client = []
    for i in range(len(sub_idxs_train)):
        datasetsize_client.append(len(sub_idxs_train[i]))  
...
def sub_local_dataset(localdataset_idxs_train):
    global args, proportion
    sub_idxs_train = { i:[] for i in range(int(args.clients/proportion)) }
    for i in range(len(localdataset_idxs_train)):
        sub_localnum = int(1/proportion)
        sub_idxs_temp = np.array_split(localdataset_idxs_train[i], sub_localnum)
        for j in range(sub_localnum):
            sub_idxs_train[i*sub_localnum + j] = sub_idxs_temp[j]
            #np.random.shuffle(sub_idxs_temp[j])
            #sub_idxs_train[j*args.clients+i] = sub_idxs_temp[j]
    # for i in range(len(sub_idxs_train)):
    #     print(len(sub_idxs_train[i]))
    return sub_idxs_train

def gen_sequence():
    global args, proportion
    sub_localnum = int(1/proportion)
    seq = { i:np.array([]) for i in range(int(args.clients/proportion)) }
    for i in range(args.clients):
        t = np.random.choice(range(i*sub_localnum, (i+1)*sub_localnum), sub_localnum, replace=False)
        for j in range(sub_localnum):
            seq[j*args.clients+i] = t[j]
    return seq
