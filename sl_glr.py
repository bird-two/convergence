...
def main():
    ...
    param_sum(global_model_client.parameters(), 'client initial weight {}'.format(args.lr))
    param_sum(global_model_server.parameters(), 'server initial weight {}'.format(args.lr))
    ...
    for r in range(rounds):
        ...
        compute_drift('client drift', global_model_client.parameters(), round_model_client.parameters())
        compute_drift('server drift', global_model_server.parameters(), round_model_server.parameters())
...
def compute_drift(tag, params1, params2):
    r'''Caculate the square of params1 and params2.
    '''
    a = []
    for p1, p2 in zip(params1, params2):
        a.append((p2-p1).data.square().sum())
    drift = sum(a)
    loginfo('{} {}'.format(tag, drift.item()))   

def param_sum(params, i=0):
    a = []
    for p in params:
        a.append(p.data.sum())
    b = sum(a)
...
# def param_diff(params1, params2, i=0):
#     r'''Caculate the sum of difference of params1 and params2.
#     '''
#     a = []
#     for p1, p2 in zip(params1, params2):
#         a.append((p1-p2).data.sum())
#     b = sum(a)
#     print(b, i)
#     return b
...
def localupdate(train_loader, model_client, model_server, criterion, device, args):
    ...
    if args.optim == 'adam':
        ...
        optimizer_client = SGD(model_client.parameters(), lr=args.lr**0.5, momentum=args.momentum, weight_decay=args.weight_decay)
        optimizer_server = SGD(model_server.parameters(), lr=args.lr**0.5, momentum=args.momentum, weight_decay=args.weight_decay)
...
def inference(train_loaders, model_client, model_server, criterion, round):
    ...
    with torch.no_grad():
        ...
        print("rounds {}'s server train acc avg: {:.2f}%, train loss avg: {:.4f}".format(round + 1, avgtops[0].avg, avglosses.avg), 'green')
...
def evaluate(val_loader, model_client, model_server, criterion, round):
    ...
    with torch.no_grad():
        ...
        print("rounds {}'s server test  acc    : {:.2f}%, test  loss    : {:.4f}".format(round + 1, tops[0].avg, losses.avg), 'red')
