...
class FileNameCurve():
    ...
    def select_files(self, raw_files):
        ...
        if self.glr == None:
            ...
            self.title = 'MNIST, 100 clients, 5 classes, local_lr={}'.format(self.llr)
            self.sl_legend = ['global_lr={}'.format(i) for i in self.choice]
            for i in self.choice:
                t = 'base sl(100 1 {}) lenet5(2) mnist(pat2-b 5.0) sgd({} 0.9 0.0001) hp(10)'.format(i, self.llr)
                if '{}{}'.format(t, '.csv') in raw_files:
                    sl_files.append(t)
                    self.sl_legend.append('local_lr={}'.format(i))
        elif self.llr == None:
            self.choice = [1e-5, 0.0001, 0.001, 0.01, 0.1]
            self.title = 'MNIST, 100 clients, 5 classes, global_lr={}'.format(self.glr)
            self.sl_legend = []
            for i in self.choice:
                t = 'base sl(100 1 {}) lenet5(2) mnist(pat2-b 5.0) sgd({} 0.9 0.0001) hp(10)'.format(self.glr, i)
                if '{}{}'.format(t, '.csv') in raw_files:
                    sl_files.append(t)
                    self.sl_legend.append('local_lr={}'.format(i))
        return sl_files
...
def plotcurve(args):
    ...
    fncurve = FileNameCurve(args.glr, args.llr)
